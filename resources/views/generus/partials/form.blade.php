...
<form method="POST" action="{{ $action }}" enctype="multipart/form-data" class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div>
            ...
            <p class="mt-1 text-sm text-slate-500">{{ $generus ? 'Data dasar generus.' : 'Data dasar generus yang akan didaftarkan.' }}</p>
        </div>
        <div class="mt-6 grid gap-5 md:grid-cols-2">
            <div>
                <label class="mb-2 block text-sm font-medium text-slate-700" for="registration_number">Nomor Induk Otomatis</label>
                <input id="registration_number" type="text" readonly value="{{ $registrationNumber }}" class="w-full rounded-lg border border-slate-200 bg-slate-100 px-3 py-2.5 text-sm text-slate-600">
                @unless ($generus)
            ...
            <div>
                ...
                <input id="full_name" name="full_name" type="text" required value="{{ old('full_name', $generus?->full_name) }}" class="w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm focus:border-amber-400 focus:outline-none focus:ring-2 focus:ring-amber-100">
            </div>
            ...
            <div>
                ...
                <select id="gender" name="gender" class="w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm focus:border-amber-400 focus:outline-none focus:ring-2 focus:ring-amber-100">
                    ...
                    <option value="laki-laki" @selected(old('gender', $generus?->gender) === 'laki-laki')>Laki-laki</option>
                    <option value="perempuan" @selected(old('gender', $generus?->gender) === 'perempuan')>Perempuan</option>
                </select>
            </div>

            <div>
                <label class="mb-2 block text-sm font-medium text-slate-700" for="birth_date">Tanggal Lahir</label>
                <input id="birth_date" name="birth_date" type="date" value="{{ old('birth_date', $generus?->birth_date?->format('Y-m-d')) }}" class="w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm focus:border-amber-400 focus:outline-none focus:ring-2 focus:ring-amber-100">
            </div>
        ...
        <div class="mt-6 grid gap-5 md:grid-cols-2">
            @foreach([
                ...
                <div>
                    ...
                    @if (in_array($field, ['record_number', 'nis'], true))
                        <input id="{{ $field }}" type="text" readonly value="{{ $field === 'nis' ? $nis : $recordNumber }}" class="w-full rounded-lg border border-slate-200 bg-slate-100 px-3 py-2.5 text-sm text-slate-600">
                        @unless ($generus)
            ...
            <div>
                ...
                <input id="sibling_count" name="sibling_count" type="number" min="0" value="{{ old('sibling_count', $generus?->sibling_count) }}" class="w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm focus:border-amber-400 focus:outline-none focus:ring-2 focus:ring-amber-100">
            </div>
        </div>
            ...
            <div>
                ...
                <select id="village_id" name="village_id" class="w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm focus:border-amber-400 focus:outline-none focus:ring-2 focus:ring-amber-100">
                    ...
                    @foreach($villages as $village)
                        <option value="{{ $village->id }}" @selected(old('village_id', $currentAssignment?->village_id) == $village->id)>{{ $village->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <select id="level_id" name="level_id" class="w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm focus:border-amber-400 focus:outline-none focus:ring-2 focus:ring-amber-100">
                    ...
                    @foreach($levels as $level)
                        <option value="{{ $level->id }}" @selected(old('level_id', $currentAssignment?->level_id) == $level->id)>{{ $level->name }}</option>
                    @endforeach
                </select>
            </div>
        ...
        <div class="mt-5">
            ...
            <textarea id="notes" name="notes" rows="3" class="w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm focus:border-amber-400 focus:outline-none focus:ring-2 focus:ring-amber-100">{{ old('notes', $generus?->notes) }}</textarea>
        </div>
