...
@section('title', 'Milestone dan Capaian')
@section('page-title', 'Milestone dan Capaian')
@section('breadcrumb', 'Program Pembinaan / Milestone')

@section('content')
    <div class="mb-6">
        <p class="text-sm font-semibold text-amber-600">Program Pembinaan</p>
        <h2 class="mt-1 text-2xl font-bold tracking-tight text-slate-950">Milestone dan Capaian</h2>
        <p class="mt-2 text-sm text-slate-500">Catat target dan pencapaian generus dalam periode tertentu.</p>
    </div>
        ...
        <div class="grid gap-6 lg:grid-cols-[minmax(18rem,0.8fr)_minmax(0,1.5fr)]">
            <div class="lg:col-span-1 rounded-xl bg-white p-6 shadow-sm border border-slate-200">
                <h2 class="text-xl font-semibold mb-4">Tambah Milestone</h2>
                <form method="POST" action="{{ route('milestones.store') }}">
                    ...
                    <div class="mb-4">
                        <label class="block text-sm font-medium mb-2" for="achievement_date">Tanggal Capaian</label>
                        <input id="achievement_date" name="achievement_date" type="date" required class="w-full rounded-lg border border-slate-300 px-3 py-2">
                    </div>
                    <div class="mb-4">
            ...
            <div class="lg:col-span-2 rounded-xl bg-white p-6 shadow-sm border border-slate-200">
                <h2 class="text-xl font-semibold mb-4">Daftar Milestone</h2>
                        ...
                        <tbody>
                            @forelse($milestones as $milestone)
                                <tr class="border-b border-slate-100">
                                    <td class="py-3 pr-4">{{ $milestone->generus?->full_name ?? '-' }}</td>
                                    <td class="py-3 pr-4">{{ $milestone->title }}</td>
                                    <td class="py-3 pr-4">{{ $milestone->status }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="3" class="py-6 text-center text-slate-500">Belum ada milestone.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
